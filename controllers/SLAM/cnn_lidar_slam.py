...
class CNNLidarSLAMController:
    ...
    def _drive(self, ranges: np.ndarray) -> None:
        """
        LiDAR-reactive navigation with narrow sector detection.

        The scan covers the full FOV. We only react to the ±30° front arc
        for collision avoidance, and use ±90° side arcs for gentle wall
        repulsion.  A stuck-counter triggers a reverse escape manoeuvre.
        """
        n = len(ranges)
        if n == 0:
            return
        ...
        # Map angular sectors to array indices.
        # Angles run from -FOV/2 (index 0) to +FOV/2 (index n-1).
        # Positive angles = left of robot forward direction in Webots.
        fov = self.lidar.getFov()           # radians (e.g. 2π for 360°)
        def _idx(angle_rad: float) -> int:
            """Convert angle offset from centre to array index."""
            frac = (angle_rad + fov / 2.0) / fov
            return int(np.clip(frac * n, 0, n - 1))

        # Front ±30°
        front = valid[_idx(-math.radians(30)):_idx(math.radians(30)) + 1]
        # Left 30°–90°
        left  = valid[_idx(math.radians(30)):_idx(math.radians(90)) + 1]
        # Right 30°–90° (negative angles)
        right = valid[_idx(-math.radians(90)):_idx(-math.radians(30)) + 1]

        front_min = float(front.min()) if len(front) > 0 else self.lidar_max_range
        left_min  = float(left.min())  if len(left)  > 0 else self.lidar_max_range
        ...
        if self._step_count % 200 == 1:
            print(
                ...
                f"front_min={front_min:.3f} left_min={left_min:.3f} right_min={right_min:.3f} "
        ...
        if front_min < self._SAFE_DIST:
            ...
            if self._stuck_counter > 40:
                # Escape: reverse and turn hard
                speed_cmd = -self._MAX_SPEED * 0.4
                steer = 0.9 if left_min > right_min else -0.9
                if self._stuck_counter > 70:
                    self._stuck_counter = 0   # reset after escape
            else:
                # Turn in place toward the clearer side
                speed_cmd = self._MAX_SPEED * 0.2
                steer = 0.9 if left_min > right_min else -0.9
        else:
            self._stuck_counter = 0
            speed_cmd = self._MAX_SPEED
            # Gentle wall repulsion
            if left_min < self._SAFE_DIST * 1.5 and left_min < right_min:
                steer = 0.35    # veer right (away from left wall)
            elif right_min < self._SAFE_DIST * 1.5:
                steer = -0.35   # veer left (away from right wall)

        # Smooth steering to avoid jerking
        self._steer_state += 0.4 * (steer - self._steer_state)

        # Record speed in m/s for IEKF odometry
        wheel_rad_per_s = speed_cmd if speed_cmd > 0 else speed_cmd
        self._cmd_speed = wheel_rad_per_s * self._WHEEL_RADIUS
