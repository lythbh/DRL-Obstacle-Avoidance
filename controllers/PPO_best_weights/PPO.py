...
class Config:
    ...
    # Inference
    inference_mode: bool = True  # If True, load saved model and run inference instead of training
    model_path: str = '../../best_model.pth'  # Path to saved model for inference (relative to controller dir)
    
    # PPO Agent
    gamma: float = 0.99  # Discount factor
    epsilon: float = 0.3  # PPO clip parameter
    learning_rate: float = 1e-4
    entropy_coef: float = 0.002  # Entropy regularization
...
class RewardComputer:
    ...
    def __init__(
        ...
        goal_stop_bonus: float = 1200.0,
        goal_hold_reward: float = 100.0,
        goal_speed_penalty: float = -600.0,
        goal_overshoot_penalty: float = -500.0,
    ):
...
class PPOAgent:
    ...
    def load_model(self, model_path: str) -> None:
        ...
        try:
            ...
            print(f"[PPO] Reward: {checkpoint.get('reward', 'unknown')}")
            print(f"[PPO] Goal episode: {checkpoint.get('goal_episode', False)}")
        except FileNotFoundError:
            print(f"[PPO] ERROR: Model file {model_path} not found!")
            raise
        except Exception as e:
...
def train(config: Optional[Config] = None) -> None:
    ...
    if config.inference_mode:
        ...
        print(f"[INFERENCE] Running inference with model: {config.model_path}")
        
        # Run a single episode for inference
        obs, _ = env.reset()
        done = False
        episode_reward = 0.0
        steps = 0
