...
class RecurrentActorCriticBase(nn.Module):
    ...
    Subclasses must override ``get_initial_state`` and ``_run_recurrent``.
    """
    ...
    def __init__(
        ...
        """Initialise feature encoders, grid encoder (CNN/MLP), and fusion network."""
        super().__init__()
    ...
    def get_initial_state(
        ...
        """Return zeroed recurrent state for the given batch size."""
    ...
    def _run_recurrent(
        ...
        """Execute recurrent forward pass with done-mask state resets."""
        raise NotImplementedError

    def _state_batch_size(self, recurrent_state: Optional[RecurrentState]) -> Optional[int]:
        """Infer batch dimension from recurrent state, handling both GRU (tensor) and LSTM (tuple)."""
    ...
    def _infer_batch_time(
        ...
        """Heuristic to split a flat or [B,T] tensor into batch and time dimensions."""
    ...
    def _prepare_tensor_component(
        ...
        """Convert raw observation component to [B,T,D] tensor with NaN sanitisation."""
    ...
    def _grid_feature_rank(self, grid_tensor: torch.Tensor) -> int:
        """Determine whether grid input is spatial (2D/3D) or flat, returning rank for reshape logic."""
    ...
    def _reshape_grid_for_cnn(self, grid: torch.Tensor, batch_size: int, seq_len: int) -> torch.Tensor:
        """Flatten grid [B,T,...] into [B*T,C,H,W] for Conv2d encoding."""
        if self.grid_shape is None:
    ...
    def _split_observation(
        ...
        """Parse observation (dict or flat array) into obstacle, pose_goal, imu, and optional grid components."""
    ...
    def _encode_observation(
        ...
        """Encode observation components through their branches and fuse into a latent [B,T,L] tensor."""
    ...
    def _prepare_done_mask(
        ...
        """Normalise done indicator to [B,T] float tensor for state-reset gating."""
